...
CodeSearchAgent = Agent(
    ...
    instructions=[
        ...
        "   - `search_batch(vec_repo_path, queries, max_results)`: 批量查询，适合需要同时搜索多个相关概念的场景"
        "   **使用原则**:"
        "   - 如果需要搜索 3+ 个相关但不同的概念（如'authentication', 'authorization', 'session management'），使用 `search_batch` 一次性查询"
        "   - 如果只是单个查询或需要根据前一次结果动态调整查询，使用 `semantic_code_search`"
        "   - 如果用户问题明确包含多个子问题（如'how does auth and db connection work'），拆分为多个查询词并使用 `search_batch`",
        "3. 根据用户意图或者主程的反馈，在数据库中进行广泛搜索，可以尝试多种关键词组合（如函数名、类名、功能描述等）。",
        "4. **相关性过滤**: 针对搜索的query，判断哪些片段是真正有用的，哪些是噪音（如测试用例、无关的工具函数）。剔除噪音。",
        "5. **上下文压缩**: 对于长函数，如果不是核心逻辑，提取其函数签名（Signature）和文档字符串（Docstring）即可。", 
        "6. 不要试图回答问题，不要担心上下文太长，尽可能多收集但只保留最相关片段。你的任务是把搜索到的所有原始代码片段完整交给 [LeadArchitect]。",
    ],
    add_history_to_context=True, 
)
